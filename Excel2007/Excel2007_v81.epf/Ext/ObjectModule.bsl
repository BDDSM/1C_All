...
Функция ОткрытьЛист(НомерЛиста) Экспорт
	...
	Индикатор = ПолучитьИндикаторПроцесса(СтрСтрок.Строки.Количество(), "Загрузка");
	...
	Для Каждого Строка Из СтрСтрок.Строки Цикл
		...
		ОбработатьИндикатор(Индикатор, СтрСтрок.Строки.Индекс(Строка));
	КонецЦикла;
...
Функция ПолучитьИндикаторПроцесса(КоличествоПроходов, ПредставлениеПроцесса = "Выполнено", ВнутреннийСчетчик = Истина,
	...
	Индикатор.Вставить("ПроцессорВывода", ПроцВывода);
	Индикатор.Вставить("ЭлементРезультата", Новый ЭлементРезультатаКомпоновкиДанных);
...
Процедура ОбработатьИндикатор(Индикатор, Счетчик = 0) Экспорт 
	...
	Если Счетчик > Индикатор.СледующийСчетчик Тогда
		...
		Если Индикатор.КоличествоПроходов > 0 Тогда
			Индикатор.ЭлементРезультата.ПроцентВывода = Счетчик / Индикатор.КоличествоПроходов * 100;
			Индикатор.ПроцессорВывода.ВывестиЭлемент(Индикатор.ЭлементРезультата);
	...
	Если Счетчик = Индикатор.КоличествоПроходов Тогда
		#Если Клиент Тогда
		Состояние("");
		Индикатор.ПроцессорВывода.ЗакончитьВывод();
		#КонецЕсли
	КонецЕсли;
